...
class FacialRecognition:
  ...
  def calibrate_center(self):
    print("Look at camera directly (centered). Press 'c' to continue when face and nose are recognized.")
  ...
  def calibrate_edge(self, motion_str):
    print("Move head slowly %s."%motion_str)
    edge_x, edge_y = 0, 0
    while (self.cap.isOpened()):
      ret, frame = self.cap.read()
      frame = cv2.resize(frame, dsize=(0, 0), fx=0.3, fy=0.3)
  ...
  def recognize_features(self):
    ...
    while self.cap.isOpened():
      ...
      frame = cv2.resize(frame, dsize=(0, 0), fx=0.3, fy=0.3)
      if not ret:
        print("Connection with camera lost, quitting.")
        sys.exit(-1)
      gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
      faces = self.face_cascade.detectMultiScale(gray, scaleFactor=1.3, minNeighbors=5)
